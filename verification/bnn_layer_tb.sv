...
module bnn_layer_tb;
    ...
    localparam int CLK_PERIOD_NS = 10;  // 10 ns = 100 MHz
    ...
    class Scoreboard;
        ...
        function void report();
            ...
            // Latency report (only for passing tests)
            $display("\n================================================================================");
            $display("LATENCY REPORT  (start pulse -> done pulse, passing tests only)");
            $display("================================================================================");
            $display("Config:  INPUTS=%0d  NEURONS=%0d  PW=%0d  PN=%0d", INPUTS, NEURONS, PW, PN);
            $display("Theoretical min cycles per inference:");
            $display("  Beats per group : %0d", BEATS);
            $display("  Neuron groups   : %0d", NEURON_GROUPS);
            $display("  Est. min cycles : ~%0d  (BEATS x NEURON_GROUPS + pipeline drain)",
                     BEATS * NEURON_GROUPS);
            if (latency_count > 0) begin
                ...
                $display("  Min latency : %0d cycles  (%0.1f ns)", latency_min,
                         real'(latency_min) * CLK_PERIOD_NS);
                $display("  Max latency : %0d cycles  (%0.1f ns)", latency_max,
                         real'(latency_max) * CLK_PERIOD_NS);
                $display("  Avg latency : %.1f cycles  (%.1f ns)", avg_lat, avg_lat_ns);
                $display("  Throughput  : %.2f inferences/us", 1000.0 / avg_lat_ns);
            end else begin
                $display("  No passing inferences recorded.");
            end
            $display("================================================================================");
        endfunction
    endclass
    ...
    //==========================================================================
    // Cycle counter — free-running, used for latency measurement
    //==========================================================================
    longint cycle_count;
    ...
    // run_test: drives one inference, checks outputs, records latency
    task run_test(input logic [INPUTS-1:0] input_vec, input string test_name);
        ...
        fork
            begin
                @(posedge clk);
                start       = 1;
                start_cycle = cycle_count;  // record cycle start was asserted
                @(posedge clk);
                start = 0;
                wait (done);
                done_cycle = cycle_count;  // record cycle done fired
                @(posedge clk);
            end
            begin
        ...
        if (golden.verify_outputs(input_vec, act, pop, error_msg)) begin
            $display("[PASS] %s  latency=%0d cycles (%0.1f ns)", test_name, elapsed,
    ...
    task automatic check_busy_protocol(input logic [INPUTS-1:0] input_vec, output bit pass);
        ...
        if (timed_out) begin
            $error("[TIMEOUT] busy protocol");
            pass = 0;
        end
        if (!busy_went_high) begin
            $error("[PROTOCOL] busy never high");
            pass = 0;
        end
    ...
    initial begin
        ...
        // ------------------------------------------------------------------
        // Final report (includes latency stats)
        // ------------------------------------------------------------------
