...
module bnn_core_tb #(
    ...
    class LatencyStats;
        ...
        function void report();
            ...
            $display("Config: INPUTS=%0d  HIDDEN1=%0d  HIDDEN2=%0d  OUTPUTS=%0d", INPUTS, HIDDEN1, HIDDEN2,
                     OUTPUTS);
            $display("        PW=%0d  PN=%0d  CLK=%0d ns", PW, PN, CLK_PERIOD_NS);
            $display("Theoretical layer cycle budgets (BEATS x GROUPS):");
            $display("  Layer 1: %0d beats x %0d groups = %0d compute cycles", MAX_BEATS_L1,
                     (HIDDEN1 + PN - 1) / PN, MAX_BEATS_L1 * ((HIDDEN1 + PN - 1) / PN));
            $display("  Layer 2: %0d beats x %0d groups = %0d compute cycles", MAX_BEATS_L2,
                     (HIDDEN2 + PN - 1) / PN, MAX_BEATS_L2 * ((HIDDEN2 + PN - 1) / PN));
            $display("  Layer 3: %0d beats x %0d groups = %0d compute cycles", MAX_BEATS_L3,
                     (OUTPUTS + PN - 1) / PN, MAX_BEATS_L3 * ((OUTPUTS + PN - 1) / PN));
            if (count > 0) begin
                avg_cycles        = real'(total_cycles) / real'(count);
                avg_ns            = avg_cycles * CLK_PERIOD_NS;
                throughput_per_us = 1000.0 / avg_ns;
                $display("Measured (%0d inferences):", count);
                ...
                $display("  Avg latency : %.1f cycles  (%.1f ns)", avg_cycles, avg_ns);
                $display("  Throughput  : %.4f inferences/us", throughput_per_us);
            end else begin
    ...
    class model_item;
        ...
        function void set_all_thresholds(int val);
            for (int l = 0; l < 3; l++)
            for (int n = 0; n < layer_neurons[l]; n++) thresholds[l][n] = THRESHOLD_W'(val);
        endfunction
    endclass
...
endtask
    ...
    task automatic run_inference(input logic [INPUTS-1:0] in_vec);
        ...
        if (LOG_INFERENCE)
            $display(
                "[%0t] Inference done  (cycle %0d)  latency=%0d cycles  result_valid=%0b",
                $realtime,
                t_done,
                elapsed,
                result_valid
    ...
    initial begin : test_sequence
        ...
        t_done  = cycle_count;
        elapsed = int'(t_done - t_start);
        $display("[INFO] Test15: Inference latency = %0d cycles  (%0.1f ns)", elapsed,
